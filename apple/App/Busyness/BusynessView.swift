            ...
            if libraries.isEmpty {
                ...
                ForEach(grouped, id: \.category) { group in
                    ...
                    BusynessGroupSection(
                        ...
                        cardSpacing: StudyLibraryCardLayout.cardSpacing
                    )
                }
            }
        }
...
struct BusynessGroupSection: View {
    ...
    var cardSpacing: CGFloat = StudyLibraryCardLayout.cardSpacing
...
struct BusynessFacilityCard: View {
    ...
    private var facilitySummary: some View {
        VStack(alignment: .leading, spacing: StudyLibraryCardLayout.stackSpacing) {
            HStack(alignment: .center, spacing: 8) {
                ...
                Image(systemName: ExpandChevron.systemName(isExpanded: isExpanded))
                    ...
                    .foregroundStyle(Color.inkMuted)
                    .frame(
                        width: StudyLibraryCardLayout.chevronSide,
                        height: StudyLibraryCardLayout.chevronSide
                    )
                    .accessibilityHidden(true)
            }

            if StudyLibraryCardLayout.showsCrowdingChrome(isOpen: effectivelyOpen) {
                HStack(alignment: .firstTextBaseline, spacing: 8) {
                    if let percent = facility.percent {
                        Text("\(percent)%")
                            .font(
                                ZotFont.face(
                                    StudyLibraryCardLayout.crowdingPercentSize,
                                    relativeTo: .title2
                                ).weight(.semibold)
                            )
                            .monospacedDigit()
                ...
                OccupancyBar(
                    ...
                    height: StudyLibraryCardLayout.occupancyBarHeight
                )
            }
        ...
        .padding(StudyLibraryCardLayout.contentPadding)
        .contentShape(Rectangle())
        .frame(minHeight: StudyLibraryCardLayout.minTapHeight, alignment: .topLeading)
    }
